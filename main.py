...
def train(config: Config, mind_corpus: MIND_Corpus):
    model = Model(config, mind_corpus.category_dict, corpus=mind_corpus)
    model.initialize()
    ...
    if config.world_size == 1:
        ...
        try:
            mp.spawn(distributed_train, args=(model, config, mind_corpus, run_index), nprocs=config.world_size, join=True)  # spawn distributed training processes
...
def dev(config: Config, mind_corpus: MIND_Corpus):
    ...
    auc, mrr, ndcg5, ndcg10 = compute_scores_with_cache(
        model, mind_corpus, config.batch_size * 8 // config.world_size, 'dev',
        dev_res_dir + '/' + model.model_name + '.txt', config.dataset,
        config=config
    )
...
def test(config: Config, mind_corpus: MIND_Corpus):
    ...
    auc, mrr, ndcg5, ndcg10 = compute_scores_with_cache(
        model, mind_corpus, config.batch_size * 4 // config.world_size, 'test',
        test_res_dir + '/' + model.model_name + '.txt', config.dataset,
        config=config
    )
...
if __name__ == '__main__':
    ...
    if config.mode == 'train':
        ...
        # For POPCORN models, build the path that includes base-encoder info
        if config.news_encoder == 'POPCORN' and config.user_encoder == 'POPCORN':
            ...
            model_name = f'POPCORN-{base_news_encoder}-{base_user_encoder}'
        else:
        ...
        # Append Test_Time / Total_Time to resource_log.txt
        resource_log_path = getattr(config, 'resource_log_path', None)
        if resource_log_path is not None and os.path.exists(resource_log_path):
            total_train_time = getattr(config, 'total_train_time', 0.0)
            total_dev_time = getattr(config, 'total_dev_time', 0.0)
            total_time = total_train_time + total_dev_time + test_time
            with open(resource_log_path, 'a', encoding='utf-8') as f:
                f.write('Test_Time:\t%.2fs\n' % test_time)
                f.write('Total_Time:\t%.2fs\n' % total_time)
